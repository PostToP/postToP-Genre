...
RUN pip install --no-cache-dir torch==2.10.0+cpu torchaudio==2.10.0+cpu \
...
RUN python -c "from src.config.config import TRANSFORMER_MODEL_NAME; print(TRANSFORMER_MODEL_NAME)" \
    && python -c "from src.config.config import TRANSFORMER_MODEL_NAME; from transformers import AutoFeatureExtractor; AutoFeatureExtractor.from_pretrained(TRANSFORMER_MODEL_NAME)"

FROM python:3.11-slim AS production
WORKDIR /app
# Install runtime binaries in the final image
RUN apt-get update && apt-get install -y --no-install-recommends \
    ffmpeg \
    && rm -rf /var/lib/apt/lists/*
